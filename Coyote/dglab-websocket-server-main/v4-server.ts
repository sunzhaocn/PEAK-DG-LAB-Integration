...
// Pairing target IDs must not be feasibly guessable on a shared LAN.
...
class RelayServer {
    ...
    onMessage(ws: ServerWebSocket<WSData>, message: string | Buffer<ArrayBuffer>) {
        ...
        if (!isMessageFrame(parsed)) {
            log('debug', `WS忽略帧 连接=${this.clientIdOf(ws)} 类型=${describeFrameType(parsed)}`);
            return;
        }
        const msg = parsed;
        ...
        if (this.controllersById.has(clientId)) {
            ...
            if (clientWs?.readyState === WebSocket.OPEN) {
                ...
                log('info', `WS转发 控制方=${clientId} 被控方=${tid} ${this.describeData(msg.data)}`);
            } else {
        ...
        if (controllerWs?.readyState === WebSocket.OPEN) {
            ...
            log('info', `WS上报 被控方=${clientId} 控制方=${this.clientIdOf(controllerWs)} ${this.describeData(msg.data)}`);
        } else {
            log('warn', `WS控制方缺失 被控方=${clientId}`);
        }
    }

    onPong(ws: ServerWebSocket<WSData>): void { this.missedWsPongs.set(ws, 0); }
    ...
    onClose(ws: ServerWebSocket<WSData>, code: number, reason: string): void {
        ...
        if (this.controllersById.has(clientId)) {
            ...
            log('info', `控制方断开 控制方=${clientId} 码=${code} 原因=${reason || '-'} 踢出=${clients?.size ?? 0}`);
            return;
        }
        ...
        if (controllerWs) {
            ...
            log('info', `被控方断开 被控方=${clientId} 控制方=${this.clientIdOf(controllerWs)} 码=${code} 原因=${reason || '-'}`);
        }
    }
    ...
    private attachClient(ws: ServerWebSocket<WSData>, clientId: string, tid: string) {
        ...
        if (!controllerWs || controllerWs.readyState !== WebSocket.OPEN) {
            ...
            log('warn', `被控方拒绝 被控方=${clientId} 目标=${tid} 原因=控制方不存在`);
            return;
        }
        const clients = this.controlledClients.get(controllerWs);
        if (!clients) {
            this.sendFrame(ws, { type: 'error', code: 'controller_not_found' });
            ws.close(CLOSE_CONTROLLER_NOT_FOUND, 'controller_not_found');
            log('error', `被控方拒绝 被控方=${clientId} 目标=${tid} 原因=控制方映射缺失`);
            return;
        }
        ...
        log('info', `被控方接入 被控方=${clientId} 控制方=${tid} 总数=${clients.size}`);
    }
...
const handleFetch = (req: Request, server: Server<WSData>): Response | undefined => {
    ...
    if (server.upgrade(req, { data: { tid } })) {
        log('debug', `WS升级 角色=${tid ? '被控方' : '控制方'} 目标=${tid ?? '-'}`);
        return;
    }
